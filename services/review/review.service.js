...
import validateReview from "./review.schema";
...
async function createNewreview(review) {
    ...
    try {
        ...
        throw new HttpError(StatusCodes.INTERNAL_SERVER_ERROR, "Can not create review");
    }
}
...
async function getAllReviews() {
    ...
    if (!reviews)
        throw new HttpError(StatusCodes.NOT_FOUND, "No reviews found");
...
async function getReviewById(id) {
    ...
    if (!review)
        throw new HttpError(StatusCodes.NOT_FOUND, "No review found");
...
async function updateReview(reviewID, newMessage, newNumberOfStars) {
    ...
    if (newMessage) {
        updatedReview = await reviewModel.
            findOneAndUpdate({ _id: reviewID }, { message: newMessage }, { new: true });
        if (!updatedReview)
            throw new HttpError(StatusCodes.NOT_FOUND, "review message update failed");
    }
    if (newNumberOfStars) {
        updatedReview = await reviewModel.
            findOneAndUpdate({ _id: reviewID }, { message: newMessage }, { new: true });
        if (!updatedReview)
            throw new HttpError(StatusCodes.NOT_FOUND, "review number of stars update failed");
